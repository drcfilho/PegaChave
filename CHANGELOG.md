...
* **Ordenação Interativa de Tabelas**: Adicionado suporte a ordenação interativa A-Z na tabela de chaves (`admin_chaves.php`). Os cabeçalhos das colunas (Sala/Ambiente, Bloco, Andar, Código, QR Hash, Descrição, Acesso Restrito e Status) são agora clicáveis e reordenam as linhas instantaneamente de forma ascendente ou descendente no próprio navegador.
