...
class RelatorioRepository extends BaseRepository {
    ...
    public function getOcorrencias($usuario_id, $chave_id, $data_inicio, $data_fim, $limite = null) {
        $sql = "
            SELECT 
                m.id, 
                m.data_retirada, 
            ...
            JOIN usuarios u ON m.usuario_id = u.id
            JOIN chaves c ON m.chave_id = c.id
            WHERE m.observacao IS NOT NULL 
              AND m.observacao != '' 
              AND (m.observacao LIKE '🚨%' OR m.observacao LIKE '%OCORRÊNCIA%')
        ";
        ...
        $sql .= " ORDER BY m.data_devolucao DESC";
